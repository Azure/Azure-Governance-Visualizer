...
# Release notes 01/04/2023 - V1.3.0:
- Add support for moving release to root of directory only when single latest release is fetched via `moveToRoot` parameter.
#>
...
param (
  ...
  [Parameter(Mandatory = $false, Position = 5, HelpMessage = 'Move the downloaded release to the root of the directory. Defaults to false.')]
  [bool]
  $moveToRoot = $false
)
...
if ($syncAllReleases -eq $false) {
  ...
  if ($null -eq $contentInReleaseDirectory) {
    ...
    if ($moveToRoot -eq $true) {
      Write-Host ''
      Write-Host '===> Moving release to root of directory/repo.' -ForegroundColor Cyan

      $contentToMoveToRoot = Get-ChildItem -Path $releaseDirectory -ErrorAction SilentlyContinue

      foreach ($item in $contentToMoveToRoot) {
        Copy-Item -Path $item -Recurse -Destination '.' -Force
      }
      Remove-Item -Path "$releaseDirectory" -Force -Recurse
    }
